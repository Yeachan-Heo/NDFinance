...
from blqt.backtest.loggers import *
from blqt.backtest.base import *
from talib import abstract as ta
import yfinance as yf

def download_data(ticker_str, start="2020-03-01"):
...
def main():
    ...
    for code in code_lst:
        ...
        data.from_pandas(df)
        data.add_array(label="momentum", array=ta.MOM(df["close"].values))
        data.add_array(label="MA200", array=ta.SMA(df["close"].values, timeperiod=20))
        data.add_array(label="MA50", array=ta.SMA(df["close"].values, timeperiod=5))
    ...
    for code in available_code_lst:
        product = FinancialProduct(code, 1, 1, 0.001515, 1, 1, 1)
        broker.add_ticker(product)

    system = BacktestSystem()

    stratagy = ActualMomentumStratagy(leverage=3)
